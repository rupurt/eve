...
describe('#emitter events', () => {
  ...
  test('can remove a registered emitter event handler', () => {
    const otherEmitter = new EventEmitter();
    const reactor = new Reactor({ logger: mockLogger });
    const helloEmitterCallback1 = vi.fn();
    reactor.onEmitter(otherEmitter, 'hello1', helloEmitterCallback1);
    const helloEmitterCallback2 = vi.fn();
  ...
  test('can remove all registered emitter event handlers for an event', () => {
    const otherEmitter = new EventEmitter();
    const reactor = new Reactor({ logger: mockLogger });
    const hello1EmitterCallback1 = vi.fn();
    reactor.onEmitter(otherEmitter, 'hello1', hello1EmitterCallback1);
    const hello1EmitterCallback2 = vi.fn();
    reactor.onEmitter(otherEmitter, 'hello1', hello1EmitterCallback2);
    const hello2EmitterCallback1 = vi.fn();
    reactor.onEmitter(otherEmitter, 'hello2', hello2EmitterCallback1);

    reactor.start();

    otherEmitter.emit('hello1', 'world');
    expect(hello1EmitterCallback1).toHaveBeenCalledTimes(1);
    expect(hello1EmitterCallback2).toHaveBeenCalledTimes(1);
    expect(hello2EmitterCallback1).toHaveBeenCalledTimes(0);
    otherEmitter.emit('hello2', 'world');
    expect(hello1EmitterCallback1).toHaveBeenCalledTimes(1);
    expect(hello1EmitterCallback2).toHaveBeenCalledTimes(1);
    expect(hello2EmitterCallback1).toHaveBeenCalledTimes(1);

    reactor.removeAllEmitterlListeners(otherEmitter, 'hello1');
    otherEmitter.emit('hello1', 'world');
    expect(hello1EmitterCallback1).toHaveBeenCalledTimes(1);
    expect(hello1EmitterCallback2).toHaveBeenCalledTimes(1);
    expect(hello2EmitterCallback1).toHaveBeenCalledTimes(1);
    ...
    expect(hello1EmitterCallback1).toHaveBeenCalledTimes(1);
    expect(hello1EmitterCallback2).toHaveBeenCalledTimes(1);
    expect(hello2EmitterCallback1).toHaveBeenCalledTimes(2);
  });
});
